    ...
    <div>
      ...
      <div className="container mx-auto px-4">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
          <div>
            <div className="text-2xl font-bold my-6 mx-auto text-center">Paid Invoices</div>
            {paidInvoices.map((invoice) => (
              <Card key={invoice.id}>
                <CardHeader>
                  <div className="flex justify-between items-center">
                    <CardTitle>Paid Invoice</CardTitle>
                    <Badge className="bg-green-500 text-white" variant="default">Paid</Badge>
                  </div>
                  <CardDescription>
          ...
          <div>
            <div className="text-2xl font-bold my-6 mx-auto text-center">Unpaid Invoices</div>
            {unpaidInvoices.map((invoice) => (
              <Card key={invoice.id}>
                <CardHeader>
                  <div className="flex justify-between items-center">
                    <CardTitle>Unpaid Invoice</CardTitle>
                    <Badge className="bg-red-500 text-white" variant="default">Unpaid</Badge>
                  </div>
                  <CardDescription>
                    <div>Invoice ID: {invoice.id}</div>
                    <div>Client: {invoice.client}</div>
                    <div>Date: {invoice.date}</div>
                    <div>Due Date: {new Date(invoice.date).toLocaleDateString()}</div>
                  </CardDescription>
                  <Button className="my-3" onClick={() => handleInvoiceClick(invoice)}>
                    See Details
                  </Button>
